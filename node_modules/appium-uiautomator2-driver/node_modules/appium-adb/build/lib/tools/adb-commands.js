...
const logcat_1 = __importDefault(require("../logcat"));
...
methods.getPlatformVersion = async function getPlatformVersion() {
    logger_js_1.default.info('Getting device platform version');
    try {
...
methods.getLocationProviders = async function getLocationProviders() {
    ...
    return lodash_1.default.includes(await this.shell(['cmd', 'location', 'is-location-enabled']), 'true')
        ? ['gps']
        : [];
};
/**
...
function decorateWriteSecureSettingsException(e) {
    if (lodash_1.default.includes(e.message, 'requires:android.permission.WRITE_SECURE_SETTINGS')) {
...
methods.setHiddenApiPolicy = async function setHiddenApiPolicy(value, ignoreError = false) {
    try {
        ...
        logger_js_1.default.info(`Failed to set setting keys '${HIDDEN_API_POLICY_KEYS}' to '${value}'. ` +
            `Original error: ${err.message}`);
    }
};
/**
...
methods.setDefaultHiddenApiPolicy = async function setDefaultHiddenApiPolicy(ignoreError = false) {
    try {
        ...
        logger_js_1.default.info(`Failed to delete keys '${HIDDEN_API_POLICY_KEYS}'. Original error: ${err.message}`);
    }
};
/**
...
methods.availableIMEs = async function availableIMEs() {
    try {
        return (0, helpers_js_1.getIMEListFromOutput)(await this.shell(['ime', 'list', '-a']));
    }
    catch (e) {
        const err = /** @type {Error} */ (e);
...
methods.enabledIMEs = async function enabledIMEs() {
    try {
        return (0, helpers_js_1.getIMEListFromOutput)(await this.shell(['ime', 'list']));
    }
    catch (e) {
        const err = /** @type {Error} */ (e);
...
methods.inputText = async function inputText(text) {
    ...
    if (escapedText !== originalStr || adbInputEscapePattern.test(originalStr)) {
        if (lodash_1.default.every(['"', `'`], (c) => originalStr.includes(c))) {
...
methods.getScreenOrientation = async function getScreenOrientation() {
    ...
    return (0, helpers_js_1.getSurfaceOrientation)(stdout);
};
/**
...
methods.broadcastAirplaneMode = async function broadcastAirplaneMode(on) {
    ...
    try {
        ...
        if (lodash_1.default.includes(err.stderr, 'SecurityException')) {
            try {
...
methods.isAnimationOn = async function isAnimationOn() {
    return (await bluebird_1.default.all(ANIMATION_SCALE_KEYS.map(async (k) => (await this.getSetting('global', k)) !== '0.0'))).includes(true);
};
/**
...
methods.setAnimationScale = async function setAnimationScale(value) {
    await bluebird_1.default.all(ANIMATION_SCALE_KEYS.map((k) => this.setSetting('global', k, value)));
};
/**
...
methods.startLogcat = async function startLogcat(opts = {}) {
    ...
    this.logcat = new logcat_1.default({
        adb: this.executable,
        debug: false,
        debugTrace: false,
        ...
        if (this._isPgrepAvailable || this._isPidofAvailable) {
            const shellCommand = this._isPgrepAvailable
                ? (this._canPgrepUseFullCmdLineSearch
                    ? ['pgrep', '-f', lodash_1.default.escapeRegExp(`([[:blank:]]|^)${name}([[:blank:]]|$)`)]
...
methods.getDeviceProperty = async function getDeviceProperty(property) {
    ...
    let val = stdout.trim();
    logger_js_1.default.debug(`Current device property '${property}': ${val}`);
    return val;
};
/**
...
methods.setDeviceProperty = async function setDeviceProperty(prop, val, opts = {}) {
    ...
    logger_js_1.default.debug(`Setting device property '${prop}' to '${val}'`);
...
methods.setHttpProxy = async function setHttpProxy(proxyHost, proxyPort) {
    ...
    if (lodash_1.default.isUndefined(proxyHost)) {
        throw new Error(`Call to setHttpProxy method with undefined proxy_host: ${proxy}`);
    }
    if (lodash_1.default.isUndefined(proxyPort)) {
...
methods.runInImeContext = async function runInImeContext(ime, fn) {
    ...
    if (originalIme === ime) {
        logger_js_1.default.debug(`The original IME is the same as '${ime}'. There is no need to reset it`);
    }
    else {
...
methods.getTimeZone = async function getTimeZone() {
    logger_js_1.default.debug('Getting current timezone');
    try {
...
methods.getDeviceIdleWhitelist = async function getDeviceIdleWhitelist() {
    ...
    logger_js_1.default.info('Listing packages in Doze whitelist');
    const output = await this.shell(['dumpsys', 'deviceidle', 'whitelist']);
    return lodash_1.default.trim(output).split(/\n/)
        .map((line) => lodash_1.default.trim(line))
        .filter(Boolean);
};
/**
...
methods.addToDeviceIdleWhitelist = async function addToDeviceIdleWhitelist(...packages) {
    if (lodash_1.default.isEmpty(packages) || await this.getApiLevel() < 23) {
        // Doze mode has only been added since Android 6
        return false;
    }
    logger_js_1.default.info(`Adding ${support_1.util.pluralize('package', packages.length)} ${JSON.stringify(packages)} to Doze whitelist`);
